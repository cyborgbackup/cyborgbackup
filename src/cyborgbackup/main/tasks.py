...
class RunJob(BaseTask):
    ...
    def build_args(self, job, **kwargs):
        ...
        if job.job_type == 'check':
            ...
            if job.client_id:
                args = ['mkdir', '-p', job.extra_vars_dict['dest_folder'], '&&', 'cd',
                        job.extra_vars_dict['dest_folder'],
                        '&&', 'borg', 'extract', '-v', '--list',
    ...
    def build_borg_cmd(self, job):
        ...
        if policy_type in ('mysql', 'postgresql', 'piped'):
            ...
            if policy_type == 'postgresql':
                ...
                if job.policy.extra_vars != '':
                    ...
                    if 'extended_postgresql' in pgsql_json and str(job.client.pk) in pgsql_json[
                        'extended_postgresql'].keys():
        ...
        if policy_type == 'proxmox':
            ...
            if 'extended_proxmox' in proxmox_json.keys() and str(job.client.pk) in proxmox_json[
                'extended_proxmox'].keys():
    ...
    def build_env(self, job, **kwargs):
        ...
        if not agentUsers.exists():
            ...
            agentUser = agentUsers.first()
            token, _ = Token.objects.get_or_create(user=agentUser)
        if token and (job.job_type == 'check' or job.job_type == 'catalog'):
            env['CYBORG_AGENT_TOKEN'] = str(token)
            try:
                _ = Setting.objects.get(key='cyborgbackup_url')
                base_url = _.value
            except Exception:
