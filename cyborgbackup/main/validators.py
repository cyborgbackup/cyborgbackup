...
def validate_pem(data, min_keys=0, max_keys=None, min_certs=0, max_certs=None):
    ...
    pem_objects, key_count, cert_count = [], 0, 0
    data = data.lstrip()

    while data:
        match, data = _match_pem_object(data, pem_obj_re)
        pem_obj_info = _process_pem_object(match, private_key_types, pem_obj_header_re)
        pem_objects.append(pem_obj_info)
        key_count, cert_count = _update_counts(pem_obj_info, key_count, cert_count)

    _validate_counts(key_count, cert_count, min_keys, max_keys, min_certs, max_certs)
    return pem_objects


def _match_pem_object(data, pem_obj_re):
    match = pem_obj_re.match(data)
    if not match:
        raise ValidationError(_('Invalid certificate or key: %s...') % data[:100])
    return match, match.group('next').lstrip()


def _process_pem_object(match, private_key_types, pem_obj_header_re):
    pem_obj_info = {'all': match.group(0), 'type': match.group('type')}
    if pem_obj_info['type'].endswith('PRIVATE KEY'):
        pem_obj_info['type'] = 'PRIVATE KEY'
        pem_obj_info['key_type'] = _get_key_type(pem_obj_info['type'], private_key_types)
    elif pem_obj_info['type'] != 'CERTIFICATE':
        raise ValidationError(_('Unsupported PEM object type: "%s"') % pem_obj_info['type'])
    pem_obj_info['data'] = match.group('data')
    pem_obj_info['b64'], pem_obj_info['bin'] = _validate_base64_data(pem_obj_info['data'], pem_obj_header_re)
    if pem_obj_info.get('key_type', '') == 'ed25519':
        pem_obj_info['key_enc'] = not pem_obj_info['bin'].startswith(b'openssh-key-v1\x00\x00\x00\x00\x04none')
    elif pem_obj_info.get('key_type', ''):
        pem_obj_info['key_enc'] = 'ENCRYPTED' in pem_obj_info['data']
    return pem_obj_info


def _get_key_type(pem_obj_type, private_key_types):
    key_type = pem_obj_type.replace('PRIVATE KEY', '').strip()
    try:
        return private_key_types[key_type]
    except KeyError:
        raise ValidationError(_('Invalid private key: unsupported type "%s"') % key_type)


def _validate_base64_data(data, pem_obj_header_re):
    base64_data = ''
    ...
    for line in data.splitlines():
        ...
        if line_continues:
            ...
            continue
        if pem_obj_header_re.match(line):
            line_continues = line.endswith('\\')
            continue
        base64_data += line
    try:
        ...
        if not decoded_data:
            raise TypeError
        return base64_data, decoded_data
    except TypeError:
        raise ValidationError(_('Invalid base64-encoded data'))


def _update_counts(pem_obj_info, key_count, cert_count):
    if pem_obj_info['type'] == 'PRIVATE KEY':
        key_count += 1
    elif pem_obj_info['type'] == 'CERTIFICATE':
        cert_count += 1
    return key_count, cert_count


def _validate_counts(key_count, cert_count, min_keys, max_keys, min_certs, max_certs):
    if key_count < min_keys:
        raise ValidationError(
            _('At least %(min)d private keys are required, only %(cnt)d provided.') % {'min': min_keys,
                                                                                       'cnt': key_count})
    if max_keys is not None and key_count > max_keys:
        raise ValidationError(
            _('No more than %(max)d private keys are allowed, %(cnt)d provided.') % {'max': max_keys, 'cnt': key_count})
    if cert_count < min_certs:
        raise ValidationError(
            _('At least %(min)d certificates are required, only %(cnt)d provided.') % {'min': min_certs,
                                                                                       'cnt': cert_count})
    if max_certs is not None and cert_count > max_certs:
        raise ValidationError(_('No more than %(max)d certificates are allowed, %(cnt)d provided.') % {'max': max_certs,
                                                                                                       'cnt': cert_count})
